...
// END custom_components/zont_local/frontend/nikas-specialized-shell.js

// BEGIN custom_components/zont_local/frontend/zont-app.js
...
(() => {
  ...
  class NikasGeneratedZont extends HTMLElement {
    ...
    _render() {
      ...
      if (!active || !this._tabs().length) return;
      const items = this._entries();
      this.shadowRoot.innerHTML = `
        <style data-zont-domain-base>
          :host{display:block;color:var(--primary-text-color,#202124);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}
          [data-zont-domain-frame]{display:block}.zont-domain-view{width:min(100%,980px);margin:0 auto}
          .zont-domain-view{width:min(100%,980px);margin:0 auto}section{margin-bottom:24px}section h2{margin:0 0 11px 3px;font-size:18px;font-weight:500}.card-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.system-card,.meter-card,.room-card,.compact-card,.ok-card,.mixer-card,.mode-card,.status-tile,.empty{background:var(--card-background-color,#fff);border:1px solid var(--divider-color,#ddd);border-radius:16px}
          ...
          @container nikas-panel (max-width:420px){.system-card{padding:12px}.system-title{font-size:16px}.meter-card{min-height:160px}.meter-value,.room-value{font-size:23px}.room-card{min-height:175px}.room-meta{font-size:9.5px}.status-tile{min-height:110px}.mode-buttons button{min-height:62px;padding:8px}}
        </style>
        <div data-zont-domain-frame>
          <div class="zont-domain-view" data-zont-domain-content>${this._content(active, items)}</div>
        </div>`;
      const diagnostics = this.shadowRoot.querySelector("[data-open-diagnostics]");
      if (diagnostics) diagnostics.onclick = () => this._selectTab("diagnostics");
      for (const button of this.shadowRoot.querySelectorAll("button[data-mode-entity]")) {
  ...
  ElementClass.prototype._systemOverviewV089 = function systemOverviewV089(items) {
    ...
    return `<div class="z82-system ${attention ? "attention" : ""} ${offline ? "offline" : ""}">
      <div class="z82-head">
        ...
        <div class="z82-online ${onlineTone}" role="status" aria-label="${esc(`${onlineText}. ${freshness}`)}"><i></i><strong>${esc(onlineText)}</strong><small class="${stale ? "stale" : freshnessUnknown || offline ? "unknown" : "current"}">${esc(freshness)}</small></div>
...
// NikaS Standard v2.2 shell adapter. The domain renderer above owns only the
// heating views; the vendored shell kit owns the host boundary and chrome.
function installV095() {
  const ElementClass = customElements.get(ELEMENT_NAME);
  if (!ElementClass || ElementClass.prototype.__zontV095) return false;

  const domainRender = ElementClass.prototype._render;
  ...
  const DOUBLE_TAP_GAP = 380;
  const CLICK_GUARD_MS = 420;
  const ZOOM_KEY = "nikas:panel-transform:v2:zont:panel";
  ...
  ElementClass.prototype._mountNikasShellV095 = function mountNikasShellV095() {
    if (this.__zontShellV095?.shell?.isConnected) {
      if (!this.__zontBoundaryCleanupV095) {
        this.__zontBoundaryCleanupV095 = createNikasShellScrollBoundaryGuard({
          host: this,
          viewport: this.__zontShellV095.viewport,
        });
      }
      return this.__zontShellV095;
    }

    domainRender.call(this);
    const root = this.shadowRoot;
    const frame = root?.querySelector("[data-zont-domain-frame]");
    const domain = frame?.querySelector("[data-zont-domain-content]");
    const active = this._activeTab();
    if (!root || !frame || !domain || !active) return null;
    const initialHtml = domain.innerHTML;

    const style = document.createElement("style");
    style.dataset.nikasShellV22 = "true";
    style.textContent = `${nikasShellV2Styles()}
      .zont-stage{position:relative;display:block;will-change:transform}
      .zont-viewport.zoomed .zont-stage{position:absolute;inset-block-start:0;inset-inline:0;margin-inline:auto}
      .zont-work-view{inline-size:min(100%,980px);min-block-size:100%;margin:0 auto}
      .zont-work-view[hidden]{display:none!important}
      .zont-scale-status{position:absolute;z-index:90;inset-inline-start:50%;inset-block-end:calc(76px + env(safe-area-inset-bottom,0px));transform:translate(-50%,10px);opacity:0;pointer-events:none;padding:9px 14px;border-radius:999px;background:rgba(20,27,34,.88);color:#fff;font-size:13px;font-weight:720;white-space:nowrap;transition:opacity .14s ease,transform .14s ease}
      .zont-scale-status.visible{opacity:1;transform:translate(-50%,0)}

      .z82-head{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(168px,44%);gap:12px!important;align-items:start!important}
      .z82-statuses{min-inline-size:0}
      .z82-online{min-inline-size:168px!important;min-block-size:58px!important;max-inline-size:100%;display:grid!important;grid-template-columns:10px minmax(0,1fr)!important;grid-template-rows:auto auto;gap:3px 11px!important;align-items:center!important;padding:12px 14px!important;border:1px solid color-mix(in srgb,var(--divider-color,#dfe3e8) 72%,transparent)!important;border-radius:18px!important;background:var(--card-background-color,#fff)!important;box-shadow:0 4px 14px rgba(0,0,0,.055)!important;white-space:nowrap}
      .z82-online i{grid-row:1/3!important;inline-size:10px!important;block-size:10px!important;border-radius:50%!important;background:currentColor!important}
      .z82-online strong{font-size:16px!important;font-weight:700!important;line-height:1.05!important}
      ...
      .z82-equipment-grid{grid-template-columns:minmax(0,.92fr) minmax(0,.92fr) minmax(0,1.16fr)!important;gap:6px!important;align-items:stretch!important}
      .z82-equipment{min-inline-size:0!important;overflow:hidden}
      .z82-dhw-card{grid-column:auto!important}
      .z82-online{border-radius:18px!important}
      .z82-head h1{font-size:25px!important}
      .z82-head p,.z82-notice,.z82-equipment h3,.z82-equipment header>span,.z82-equipment h3 span,.z82-row,.z82-row strong,.z82-dhw-temperature strong,.z82-dhw-temperature small,.z82-hot-water span,.z82-circulation-loop span,.z82-cold-water span,.z82-hot-water strong,.z82-circulation-loop strong,.z82-cold-water strong,.z82-circulation-loop small,.z82-hydro strong,.z82-hydro-values span,.z82-circuit-card h3,.z82-circuit-device span,.z82-mixer span,.z82-circuit-device strong,.z82-mixer strong,.z82-circuit-values span,.z82-circuit-values strong,.z82-metric span,.z82-metric strong,.z82-metric small,.z82-node>strong,.z82-node span,.z82-node small,.z82-mode strong,.z82-mode span{font-size:12px!important}
      ...
      @container nikas-panel (max-width:520px){
        .z82-system,.z82-section{padding:10px!important}.z82-head h1{font-size:22px!important}.z82-head p{font-size:12px!important}
        .z82-equipment-grid{grid-template-columns:minmax(0,.92fr) minmax(0,.92fr) minmax(0,1.16fr)!important;gap:5px!important}.z82-equipment{padding:7px!important}.z82-dhw-card{grid-column:auto!important}.z82-equipment header{min-height:38px!important}
        .z82-boiler-card{padding-inline:6px!important}.z82-boiler-card .z82-row{display:grid!important;grid-template-columns:minmax(0,1fr);gap:2px!important}.z82-boiler-card .z82-row strong{text-align:start!important;overflow-wrap:anywhere}.z82-boiler-visual{height:102px!important}.z82-boiler-art{width:62px!important;height:96px!important}.z82-flame{--mdc-icon-size:20px!important}
        .z82-dhw-schematic{height:174px!important;min-height:174px!important;overflow:hidden}.z82-dhw-schematic .z82-tank{left:4%!important;top:31px!important;width:46px!important;height:124px!important}.z82-hot-pipe{left:calc(4% + 46px)!important;right:5%!important;top:45px!important}.z82-loop-branch{left:52%!important;top:45px!important;height:42px!important}.z82-loop-vertical{left:52%!important;top:87px!important;height:36px!important}.z82-loop-return{left:calc(4% + 46px)!important;right:48%!important;top:121px!important}.z82-cold-pipe{left:calc(4% + 19px)!important;right:5%!important;top:164px!important}.z82-loop-pump{left:52%!important;top:105px!important;width:24px!important;height:24px!important}.z82-loop-pump ha-icon{--mdc-icon-size:16px!important}
        .z82-hot-water,.z82-circulation-loop,.z82-cold-water{right:0!important;width:64px!important}.z82-hot-water{top:27px!important}.z82-circulation-loop{top:76px!important}.z82-cold-water{bottom:0!important}
        .z82-hydro-stage{height:42px!important;margin:42px 5% 48px!important}.z82-hydro strong{left:37%!important}.z82-hydro-values{right:8px!important}
        .z82-circuits-grid{grid-template-columns:1fr!important;gap:8px!important}.z82-circuit-card{padding:12px!important}.z82-metrics,.z82-nodes,.z82-modes{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important}.z82-metric,.z82-node,.z82-mode{min-height:92px!important}.z82-legend{gap:8px 12px!important}
      }
      @container nikas-panel (max-width:359px){
        .z82-head{grid-template-columns:minmax(0,1fr)!important}.z82-statuses{grid-column:1}.z82-online{inline-size:100%;min-inline-size:0!important}
        .z82-equipment-grid{grid-template-columns:minmax(0,.92fr) minmax(0,.92fr) minmax(0,1.16fr)!important}.z82-equipment{padding-inline:5px!important}.z82-equipment h3{font-size:12px!important}.z82-boiler-art{width:52px!important;height:82px!important}.z82-boiler-visual{height:88px!important}.z82-dhw-schematic{height:160px!important;min-height:160px!important}
      }
      @media(prefers-reduced-motion:reduce){.zont-scale-status{transition:none}}
    `;
    root.append(style);

    const shell = document.createElement("div");
    shell.id = "zont-shell-v095";
    shell.className = "nikas-shell";
    shell.innerHTML = `
      <header class="nikas-shell__header">
        <button id="zont-menu" class="nikas-shell__side-action" type="button" aria-label="Открыть меню Home Assistant"><ha-icon icon="mdi:menu"></ha-icon></button>
        <button id="zont-title" class="nikas-shell__title" type="button" aria-label="Вернуться в исходную базовую панель NikaS"><strong>Отопление</strong><small>UI v${UI_VERSION}</small></button>
        <button id="zont-refresh" class="nikas-shell__side-action nikas-shell__side-action--right" type="button" aria-label="Обновить"><ha-icon icon="mdi:refresh"></ha-icon></button>
      </header>
      <div id="zont-work" class="zont-viewport nikas-shell__viewport" role="main" aria-label="Рабочая область панели Отопление. Масштабирование двумя пальцами">
        <div id="zont-stage" class="zont-stage nikas-shell__canvas nikas-shell__content"></div>
      </div>
      <div id="zont-tabs" class="nikas-shell__tabs" role="navigation" aria-label="Основные разделы" style="--nikas-shell-tab-count:5"></div>
      <div id="zont-scale-status" class="zont-scale-status" role="status" aria-live="polite">Масштаб 100%</div>`;
    frame.replaceWith(shell);

    const viewport = shell.querySelector("#zont-work");
    const stage = shell.querySelector("#zont-stage");
    const nav = shell.querySelector("#zont-tabs");
    const initialView = document.createElement("div");
    initialView.className = "zont-work-view";
    initialView.dataset.view = active.id;
    initialView.innerHTML = initialHtml;
    stage.append(initialView);
    ...
    for (const tab of this._tabs()) {
      ...
      button.onclick = () => this._selectTab(tab.id);
      nav.append(button);
    ...
    this.__zontShellV095 = {
      shell,
      viewport,
      stage,
      refresh,
      navButtons,
      views: new Map([[active.id, initialView]]),
      active: active.id,
    };
    this._bindWorkActionsV095(initialView);
    this._installZoomV095();
    this.__zontBoundaryCleanupV095 = createNikasShellScrollBoundaryGuard({ host: this, viewport });
    return this.__zontShellV095;
  };

  ElementClass.prototype._bindWorkActionsV095 = function bindWorkActionsV095(view) {
  ...
  ElementClass.prototype._patchViewV095 = function patchViewV095(view, html) {
    ...
    morph(view, fresh);
    this._bindWorkActionsV095(view);
  };

  ElementClass.prototype._render = function renderV095() {
    let shell = this.__zontShellV095;
    if (!shell?.shell?.isConnected) shell = this._mountNikasShellV095();
    if (!shell) return;
    const active = this._activeTab();
    if (!active) return;
    const items = this._entries();
    ...
    if (!view) {
      ...
      view.className = "zont-work-view";
      view.dataset.view = active.id;
      view.hidden = true;
      shell.stage.append(view);
      shell.views.set(active.id, view);
    }
    this._patchViewV095(view, this._content(active, items));
    ...
    shell.refresh.disabled = Boolean(this._loading);
    if (changed) this.__zontZoomV095?.onTabChange();
    else this.__zontZoomV095?.clamp();
  };

  ElementClass.prototype._installZoomV095 = function installZoomV095() {
    const shell = this.__zontShellV095;
    if (!shell || this.__zontZoomV095?.viewport === shell.viewport) return;
    const { viewport, stage } = shell;
    let state = { scale: 1, x: 0, y: 0 };
    try {
      const stored = JSON.parse(localStorage.getItem(ZOOM_KEY) || "null");
      if (stored) {
        state = {
          scale: clamp(finite(stored.scale, 1), SCALE_MIN, SCALE_MAX),
          x: finite(stored.x),
          y: finite(stored.y),
        };
      }
    } catch (_error) { /* stored zoom is optional */ }
    ...
    const bounds = (scale = state.scale) => {
      if (scale <= 1) return { minX: 0, minY: 0 };
      const width = Math.max(stage.offsetWidth, stage.scrollWidth, 1);
      const height = Math.max(stage.offsetHeight, stage.scrollHeight, 1);
      return {
        minX: Math.min(0, viewport.clientWidth - width * scale),
        minY: Math.min(0, viewport.clientHeight - height * scale),
      };
    };
    const normalize = (scale, x, y) => {
      const nextScale = clamp(finite(scale, 1), SCALE_MIN, SCALE_MAX);
      if (nextScale <= 1) return { scale: nextScale, x: 0, y: 0 };
      const edge = bounds(nextScale);
      return {
        scale: nextScale,
        x: clamp(finite(x), edge.minX, 0),
        y: clamp(finite(y), edge.minY, 0),
      };
    };
    const apply = (persist = false) => {
      state = normalize(state.scale, state.x, state.y);
      const zoomed = state.scale > 1.0001;
      viewport.classList.toggle("zoomed", zoomed);
      stage.style.transform = Math.abs(state.scale - 1) < .0001
        ? "none"
        : `translate3d(${state.x}px,${state.y}px,0) scale(${state.scale})`;
      if (zoomed) { viewport.scrollLeft = 0; viewport.scrollTop = 0; }
      if (persist) save();
    };
    const showScaleStatus = () => {
      const status = shell.shell.querySelector("#zont-scale-status");
      if (!status) return;
      status.classList.add("visible");
      clearTimeout(this.__zontScaleTimerV095);
      this.__zontScaleTimerV095 = setTimeout(() => status.classList.remove("visible"), 1100);
    };
    ...
    const resetPosition = () => {
      viewport.scrollLeft = 0;
      viewport.scrollTop = 0;
      state = normalize(state.scale, 0, 0);
      apply(true);
    };
    const cancelPendingHold = (target) => {
    ...
    const touchStart = (event) => {
      if (event.touches.length === 1) {
        ...
        gesture = {
          startedAt: performance.now(), moved: false, multi: false,
          startX: touch.clientX, startY: touch.clientY,
          startState: { ...state }, target: event.composedPath?.()[0] || event.target,
        };
        return;
      }
      ...
      const localX = midpoint.x - rect.left - stage.offsetLeft;
      const localY = midpoint.y - rect.top - stage.offsetTop;
      const nativeScrollY = state.scale <= 1 ? viewport.scrollTop : 0;
      gesture = {
        ...(gesture || {}),
        startedAt: gesture?.startedAt || performance.now(),
        moved: false,
        multi: true,
        distance: Math.max(1, touchDistance(event.touches)),
        scale: state.scale,
        contentX: (localX - state.x) / state.scale,
        contentY: (localY + nativeScrollY - state.y) / state.scale,
        midX: midpoint.x,
        midY: midpoint.y,
        target: gesture?.target || event.target,
      };
      cancelPendingHold(gesture.target);
      suppressClicksUntil = Date.now() + CLICK_GUARD_MS;
      if (event.cancelable) event.preventDefault();
    };
    const touchMove = (event) => {
      if (!gesture) return;
      if (gesture.multi && event.touches.length === 2) {
        const midpoint = touchMidpoint(event.touches);
        const currentDistance = Math.max(1, touchDistance(event.touches));
        const travel = Math.hypot(midpoint.x - gesture.midX, midpoint.y - gesture.midY);
        if (!gesture.moved
          && Math.abs(currentDistance - gesture.distance) < PAN_THRESHOLD
          && travel < PAN_THRESHOLD) return;
        gesture.moved = true;
        const rect = viewport.getBoundingClientRect();
        const localX = midpoint.x - rect.left - stage.offsetLeft;
        const localY = midpoint.y - rect.top - stage.offsetTop;
        const scale = clamp(gesture.scale * currentDistance / gesture.distance, SCALE_MIN, SCALE_MAX);
        if (scale > 1) viewport.scrollTop = 0;
        state = normalize(
          scale,
          localX - gesture.contentX * scale,
          localY - gesture.contentY * scale,
        );
        apply();
        cancelPendingHold(gesture.target);
        suppressClicksUntil = Date.now() + CLICK_GUARD_MS;
      ...
      if (state.scale <= 1 || event.touches.length !== 1 || gesture.multi) return;
      const touch = event.touches[0];
      const deltaX = touch.clientX - gesture.startX;
      const deltaY = touch.clientY - gesture.startY;
      if (!gesture.moved && Math.hypot(deltaX, deltaY) < PAN_THRESHOLD) return;
      if (!gesture.moved) cancelPendingHold(gesture.target);
      gesture.moved = true;
      state = normalize(
        gesture.startState.scale,
        gesture.startState.x + deltaX,
        gesture.startState.y + deltaY,
      );
      apply();
      suppressClicksUntil = Date.now() + CLICK_GUARD_MS;
      if (event.cancelable) event.preventDefault();
    };
    const touchEnd = (event, cancelled = false) => {
      if (!gesture) return;
      if (cancelled) gesture.moved = true;
      if (event.touches.length) return;
      const completed = gesture;
      const now = performance.now();
      const twoFingerTap = !cancelled && completed.multi && !completed.moved
        && now - completed.startedAt <= TAP_DURATION;
      if (completed.moved && state.scale >= SNAP_MIN && state.scale <= SNAP_MAX) {
        reset(true);
      } else if (twoFingerTap) {
        if (now - lastTwoFingerTap <= DOUBLE_TAP_GAP) {
          lastTwoFingerTap = 0;
          reset(true);
        } else lastTwoFingerTap = now;
      } else apply(true);
      if (completed.moved || completed.multi) suppressClicksUntil = Date.now() + CLICK_GUARD_MS;
      gesture = null;
    };
    const touchCancel = (event) => touchEnd(event, true);
    ...
    viewport.addEventListener("touchstart", touchStart, { capture: true, passive: false });
    viewport.addEventListener("touchmove", touchMove, { capture: true, passive: false });
    viewport.addEventListener("touchend", touchEnd, { capture: true, passive: false });
    viewport.addEventListener("touchcancel", touchCancel, { capture: true, passive: false });
    viewport.addEventListener("click", clickGuard, true);
    viewport.addEventListener("contextmenu", clickGuard, true);
    const observer = typeof ResizeObserver === "function" ? new ResizeObserver(() => apply(true)) : null;
    ...
    this.__zontZoomV095 = {
      viewport,
      clamp: () => apply(),
      onTabChange: resetPosition,
      reset,
      cleanup: () => {
        viewport.removeEventListener("touchstart", touchStart, true);
        viewport.removeEventListener("touchmove", touchMove, true);
        viewport.removeEventListener("touchend", touchEnd, true);
        viewport.removeEventListener("touchcancel", touchCancel, true);
        viewport.removeEventListener("click", clickGuard, true);
        viewport.removeEventListener("contextmenu", clickGuard, true);
        observer?.disconnect();
      },
    };
    apply(true);
  ...
  ElementClass.prototype.disconnectedCallback = function disconnectedV095() {
    this.__zontZoomV095?.cleanup();
    this.__zontZoomV095 = null;
    this.__zontBoundaryCleanupV095?.();
    this.__zontBoundaryCleanupV095 = null;
    clearTimeout(this.__zontScaleTimerV095);
    return legacyDisconnected?.call(this);
  };
  ElementClass.prototype.__zontV095 = true;
  return true;
}

if (!installV095()) customElements.whenDefined(ELEMENT_NAME).then(() => installV095());
// END custom_components/zont_local/frontend/zont-app.js
